...
Vscode插件
Vscode扩展中，搜索：Remote Development，它其实是一个Extension Packs,实际包括三个包：Dev Containers,
Remote - SSH,WSL 三个。我们本次主要使用前面两个，也可以单独安装前面两个。
- Remote - SSH ：远程开发的基础插件，可以通过ssh连接远程主机进行开发
- Dev Containers ：容器开发插件，基于Docker容器
